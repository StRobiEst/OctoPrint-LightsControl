...
__author__ = "Robert Ohak <strobiest@gmail.com>"
__license__ = "GNU Affero General Public License http://www.gnu.org/licenses/agpl.html"
__copyright__ = "Copyright (C) 2018 Robert Ohak - Released under terms of the AGPLv3 License"
...
class LightsControl(octoprint.plugin.StartupPlugin,
    ...
    def __init__(self):
        ...
        self._noSensing_isLightsOn = False
        self._check_lights_state_thread = None
        self._check_lights_state_event= threading.Event()
    ...
    def on_settings_initialized(self):
        ...
        if self.sensingMethod == 'INTERNAL':
            self._logger.info("Using internal tracking for Lights on/off state.")
        elif self.sensingMethod == 'GPIO':
            self._logger.info("Using GPIO for tracking Lights on/off state.")
        elif self.sensingMethod == 'SYSTEM':
            self._logger.info("Using System Commands for tracking Lights on/off state.")
            
        if self.switchingMethod == 'GPIO' or self.sensingMethod == 'GPIO':
            self._configure_gpio()

        self._check_lights_state_thread = threading.Thread(target=self._check_lights_state)
        self._check_lights_state_thread.daemon = True
        self._check_lights_state_thread.start()
    ...
    def _configure_gpio(self):
        ...
        if self.sensingMethod == 'GPIO':
            self._logger.info("Using GPIO sensing to determine Lights on/off state.")
    ...
    def check_lights_state(self):
        self._check_lights_state_event.set()

    def _check_lights_state(self):
        while True:
            old_isLightsOn = self.isLightsOn

            if self.sensingMethod == 'GPIO':
                if not self._hasGPIO:
                    return

                self._logger.debug("Polling Lights state...")
                ...
                if r==1:
                    ...
                    new_isLightsOn = False

                if self.invertsenseGPIOPin:
                    new_isLightsOn = not new_isLightsOn

                self.isLightsOn = new_isLightsOn
            elif self.sensingMethod == 'SYSTEM':
                new_isLightsOn = False
                ...
                self.isLightsOn = self._noSensing_isLightsOn
            else:
                return
            
            self._logger.debug("isLightsOn: %s" % self.isLightsOn)

            if (old_isLightsOn != self.isLightsOn) and self.isLightsOn:
                self._start_idle_timer()
            elif (old_isLightsOn != self.isLightsOn) and not self.isLightsOn:
                self._stop_idle_timer()

            self._plugin_manager.send_plugin_message(self._identifier, dict(hasGPIO=self._hasGPIO, isLightsOn=self.isLightsOn))

            self._check_lights_state_event.wait(5)
            self._check_lights_state_event.clear()

    def _start_idle_timer(self):
        self._stop_idle_timer()
        
        if self.powerOffWhenIdle and self.isLightsOn:
    ...
    def _idle_poweroff(self):
        ...
        self._logger.info("Idle timeout reached after %s minute(s). Turning heaters off prior to shutting off Lights." % self.idleTimeout)
        if self._wait_for_heaters():
            self._logger.info("Heaters below temperature.")
            self.turn_lights_off()
        else:
            self._logger.info("Aborted Lights shut down due to activity.")
    ...
    def _wait_for_heaters(self):
        ...
        while True:
            ...
            self._logger.info("Waiting for heaters(%s) before shutting off Lights..." % ', '.join(heaters_above_waittemp))
            time.sleep(5)
        ...
        if gcode:
            if self.enablePseudoOnOff:
                if gcode == self.pseudoOnGCodeCommand:
                    self.turn_lights_on()
                    comm_instance._log("LightsControl: ok")
                    skipQueuing = True
            ...
            if (not self.isLightsOn and self.autoOn and (gcode in self._autoOnTriggerGCodeCommandsArray)):
                self._logger.info("Auto-On - Turning Lights On (Triggered by %s)" % gcode)
                self.turn_lights_on()

            if self.powerOffWhenIdle and self.isLightsOn and not self._skipIdleTimer:
    ...
    def turn_lights_on(self):
        if self.switchingMethod == 'GCODE' or self.switchingMethod == 'GPIO' or self.switchingMethod == 'SYSTEM':
            self._logger.info("Switching Lights On")
            if self.switchingMethod == 'GCODE':
                self._logger.debug("Switching Lights On Using GCODE: %s" % self.onGCodeCommand)
                self._printer.commands(self.onGCodeCommand)
            elif self.switchingMethod == 'SYSTEM':
                self._logger.debug("Switching Lights On Using SYSTEM: %s" % self.onSysCommand)
                ...
                self._logger.debug("Switching Lights On Using GPIO: %s" % self.onoffGPIOPin)
                if not self.invertonoffGPIOPin:
            ...
            if self.sensingMethod not in ('GPIO','SYSTEM'):
                self._noSensing_isLightsOn = True
         
            time.sleep(0.1 + self.postOnDelay)
            self.check_lights_state()
        
    def turn_lights_off(self):
        if self.switchingMethod == 'GCODE' or self.switchingMethod == 'GPIO' or self.switchingMethod == 'SYSTEM':
            self._logger.info("Switching Lights Off")
            if self.switchingMethod == 'GCODE':
                self._logger.debug("Switching Lights Off Using GCODE: %s" % self.offGCodeCommand)
                self._printer.commands(self.offGCodeCommand)
            elif self.switchingMethod == 'SYSTEM':
                self._logger.debug("Switching Lights Off Using SYSTEM: %s" % self.offSysCommand)
                ...
                self._logger.debug("Switching Lights Off Using GPIO: %s" % self.onoffGPIOPin)
                if not self.invertonoffGPIOPin:
                    pin_output=GPIO.LOW
                else:
                    pin_output=GPIO.HIGH
    ...
    def on_api_command(self, command, data):
        ...
        if command == 'turnLightsOn':
            self.turn_lights_on()
        elif command == 'turnLightsOff':
            self.turn_lights_off()
            ...
            if self.isLightsOn:
                ...
                self.turn_lights_on()
        elif command == 'getLightsState':
            return jsonify(isLightsOn=self.isLightsOn)
    ...
    def get_settings_defaults(self):
        return dict(
            ...
            onGCodeCommand = 'M355 S1', 
            offGCodeCommand = 'M355 S0', 
            onSysCommand = '',
            offSysCommand = '',
            enablePseudoOnOff = False,
            pseudoOnGCodeCommand = 'M355 S1',
            pseudoOffGCodeCommand = 'M355 S0',
            postOnDelay = 0.0,
    ...
    def get_assets(self):
        return {
            "js": ["js/lightscontrol.js"],
            "less": ["less/lightscontrol.less"],
            "css": ["css/lightscontrol.min.css"]
    ...
    def get_update_information(self):
        return dict(
            lightscontrol=dict(
                displayName="Lights Control",
                displayVersion=self._plugin_version,

                # version check: github repository
                type="github_release",
                user="strobiest",
                repo="OctoPrint-ligtsControl",
                current=self._plugin_version,

                # update method: pip w/ dependency links
                pip="https://github.com/strobiest/OctoPrint-LightsControl/archive/{target_version}.zip"
            )
        )
...
def __plugin_load__():
    ...
    __plugin_implementation__ = LightsControl()
